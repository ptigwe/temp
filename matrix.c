...
void matrix_set_const(matrix_t *A, double c)
{
    int i, j;
    for (i = 0; i < A->nrows; ++i)
        for (j = 0; j < A->ncols; ++j)
...
void matrix_copy_to_offset(matrix_t *dst, matrix_t *src, int roff, int coff)
{
    int i, j;
    for (i = 0; i < src->nrows; ++i) 
        for (j = 0; j < src->ncols; ++j)
            dst->data[i + roff][j + coff] = src->data[i][j];
}
...
void matrix_read(matrix_t *mat, FILE *in)
{
    ...
    for (i = 0; i < mat->nrows; ++i)
        for (j = 0; j < mat->ncols; ++j)
            fscanf(in, "%le", &mat->data[i][j]);
}

void matrix_write(matrix_t *mat, FILE *out)
{
    int i, j;
    for (i = 0; i < mat->nrows; ++i){
        for (j = 0; j < mat->ncols; ++j)
            fprintf(out, "%lf ", mat->data[i][j]);
        fprintf(out, "\n");
    }
}
...
void matrix_add_dst(matrix_t *A, matrix_t *B, matrix_t *dst)
{
...
void matrix_add_const_in(matrix_t *A, double c)
{
    int i, j;
    for (i = 0; i < A->nrows; ++i)
        for (j = 0; j < A->ncols; ++j)
...
void matrix_mul_const_in(matrix_t *A, double c)
{
    int i, j;
    for (i = 0; i < A->nrows; ++i)
        for (j = 0; j < A->ncols; ++j)
